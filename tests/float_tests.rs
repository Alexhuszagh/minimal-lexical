...
fn check_round_to_f32(mant: u64, exp: i32, r_mant: u64, r_exp: i32) {
    ...
    x.round_to_native::<f32, _>(rounding::round_nearest_tie_even);
    assert_eq!(
        x,
        float::ExtendedFloat {
            mant: r_mant,
...
fn check_round_to_f64(mant: u64, exp: i32, r_mant: u64, r_exp: i32) {
    ...
    x.round_to_native::<f64, _>(rounding::round_nearest_tie_even);
    assert_eq!(
        x,
        float::ExtendedFloat {
            mant: r_mant,
...
fn assert_normalized_eq(mut x: float::ExtendedFloat, mut y: float::ExtendedFloat) {
    x.normalize();
    y.normalize();
    assert_eq!(x, y);
}
...
fn from_float() {
    ...
    for value in values.iter() {
        assert_normalized_eq(
            float::ExtendedFloat::from_float(*value),
            float::ExtendedFloat::from_float(*value as f64),
        );
    }
}
...
fn check_mul(a: float::ExtendedFloat, b: float::ExtendedFloat, c: float::ExtendedFloat) {
    let r = a.mul(&b);
    assert_eq!(r, c);
}
...
fn mul_test() {
    ...
    let a = float::ExtendedFloat {
        mant: 13164036458569648128,
        exp: -213,
    };
    let b = float::ExtendedFloat {
        mant: 9223372036854775808,
        exp: -62,
    };
    let c = float::ExtendedFloat {
    ...
    // Check with integers
    // 64-bit mantissa
    let mut a = float::ExtendedFloat {
        mant: 10,
        exp: 0,
    };
    let mut b = float::ExtendedFloat {
        mant: 10,
        exp: 0,
    };
    a.normalize();
...
fn check_imul(mut a: float::ExtendedFloat, b: float::ExtendedFloat, c: float::ExtendedFloat) {
    a.imul(&b);
    assert_eq!(a, c);
}
